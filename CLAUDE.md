...
- 檔名格式：`NNNN-Title-With-Dashes.md`，`NNNN` 是 **4 位數補零的真實 LeetCode 題號**（由 `newproblem` 的 `sanitize_filename` 產生）。題號要正確：例如 Two Sum II - Input Array Is Sorted 是 **167**，而 128 是 Longest Consecutive Sequence——下手前先確認題號沒填錯 / 沒跟既有檔案撞號。
...
- 使用者通常會「貼一份 C++ 解法 + 問回饋 / 有沒有更好寫法」。**先討論、得到認可後才改筆記**；但若使用者明確說「補 memo / 整理這篇」，就直接動手。
