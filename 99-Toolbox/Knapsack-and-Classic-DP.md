...
全部對拍驗過：0/1 背包對照 2ⁿ 暴力枚舉、完全背包對照 DP 定義式、有界背包對照展開後的暴力、LIS 兩種定義各對照 O(n²)，合計 3000 + 30000 組全過；列舉版另外對照計數版的方案數（4 組候選 × target 1..40）全數吻合。
...
前兩節都在把答案**摺成一個標量**——`max` 摺成最優值、`+` 摺成方案數。摺疊正是 DP 能省事的原因：`dp[j]` 不管背後有多少種走法，永遠只佔 O(1) 空間。一旦題目要的是**所有方案本身**（[[0039-Combination-Sum]]），這個前提就沒了。

骨架照樣能寫，`dp[t]` 從「一個數」變成「一堆組合」：

```cpp
// 物品在外層 ⇒ 組合；正序 ⇒ 可重複取。規則和計數版一模一樣
vector<vector<vector<int>>> dp(target + 1);
dp[0].push_back({});                   // 湊出 0 只有空組合一種
for (int c : items) {
  for (int t = c; t <= target; ++t) {
    for (auto comb : dp[t - c]) {      // by value 複製一份再接上 c
      comb.push_back(c);
...
能跑，而且答案對——`dp[target].size()` 恆等於計數版算出的方案數。但它是這個家族裡**唯一該被放棄**的寫法。

> [!important] 輸出量本身是指數級時，DP 沒有東西可以壓
> `dp` 表的每一格現在要實際存下整串 `vector`，狀態大小不再是 O(1)，而轉移變成大量的深複製。更關鍵的是：回溯只維護**當前這一條路徑**（`push_back` / `pop_back` 共用同一個緩衝），DP 卻要同時把**所有中間容量的所有組合**留在記憶體裡，即使其中絕大多數永遠接不到 `target`。
>
> 實測（`items` = 12 個質數、`target = 40`、302 組解，`g++ -std=c++20 -O2`）：DP 列舉 0.172 ms，回溯 0.017 ms，**慢約 10 倍**。細節見 [[0039-Combination-Sum]]。

所以同一個「湊出 target」的骨架，該用什麼取決於**輸出形狀**，不是題目長相：

| 問法 | 合併運算 | 解法 |
| --- | --- | --- |
| **最少／最多**幾件 | `min` / `max` | 背包 DP → [[0322-Coin-Change]] |
| 有**幾種**方案 | `+` | 計數 DP → [[0518-Coin-Change-II]]、[[0377-Combination-Sum-IV]] |
| 列出**所有**方案 | 摺不了 | 回溯 → [[0039-Combination-Sum]] |

> [!tip] 反過來也成立：看到回溯題，先問「如果只問數量呢」
> 那通常就是同一題的 DP 版，而且往往是另一道真實存在的題目。0039 → 0518 就是這組對照。面試時主動點出這件事，等於免費展示你看得出題型的骨架。
...
- [[0039-Combination-Sum]] — 完全背包的列舉版，摺不成標量，只能回溯
